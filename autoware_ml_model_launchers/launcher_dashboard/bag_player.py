...
class BagPlayerManager:
    ...
    def start(
        ...
        resolved_path = Path(str(bag_path)).expanduser()
        if not resolved_path.exists():
            raise ValueError(f"bag path does not exist: {resolved_path}")
    ...
    def status(self, force_service_refresh: bool = False) -> dict[str, Any]:
        ...
        return player.to_json(service_names)
